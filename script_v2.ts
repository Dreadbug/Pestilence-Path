...
class Character {
  ...
  constructor(health: number = 2, name: string) {
    this.health = health;
    this.name = name;
  }
}
...
class Party {
  ...
  constructor(
    player: Character = new Character(2, "You"),
    wife: Character = new Character(2, "Petra"),
    son: Character = new Character(2, "Gunther"),
    daughter: Character = new Character(2, "Margeratta"),
...
class WeatherController {
  private summerTemps: number[] = [
    23, 20, 30, 23, 19, 17, 12, 18, 21, 22, 20, 26, 25, 32, 29, 27, 24,
  ];
  private fallTemps: number[] = [
    15, 12, 21, 15, 13, 12, 9, 7, 6, 10, 11, 14, 16, 23, 16, 20, 21,
  ];
  private winterTemps: number[] = [
    -3, 5, 2, 8, 6, 1, -7, -5, -9, -4, -2, -1, -6, 7, 0, 9, 10,
  ];
  private springTemps: number[] = [
    14, 11, 20, 12, 11, 7, 2, 3, 0, 8, 9, 15, 13, 16, 10, 17, 14,
  ];
...
let numClicks = 0;
function changeTitleText(): void {
  const storyText: HTMLElement | null = document.getElementById("storytext");
  if (storyText !== null) {
    numClicks += 1;
    if (numClicks == 1) {
      storyText.innerHTML =
        "...Medieval Europe. 1347. A time of sorrow and suffering...";
      document.body.style.backgroundImage = "url('assets/suffering.png')";
    } else if (numClicks == 2) {
      storyText.innerHTML =
        "...the Plague ravages the wicked and the righteous alike...";
    } else if (numClicks == 3) {
      storyText.innerHTML = "...no one is safe from its evil influence...";
      document.body.style.backgroundImage = "url('assets/no one.png')";
    } else if (numClicks == 4) {
      storyText.innerHTML =
        "...someone needs to bring the light to this forsaken place...";
